...
async function query(filterBy = {}) {
  ...
  const collection = await dbService.getCollection("programs");
  try {
...
async function remove(programId) {
  const collection = await dbService.getCollection("programs");
  try {
    await collection.deleteOne({ _id: programId });
  } catch (err) {
    console.log(`ERROR: cannot remove room ${programId}`);
    throw err;
  }
}

async function update(programs) {
  const collection = await dbService.getCollection("programs");
  try {
